...
module Sweatshop
  ...
  describe TestWorker do
    ...
    let(:trace_headers) { {Telemetry::Tracer::TRACE_HEADER_KEY => 1233467890,
                           Telemetry::Tracer::SPAN_HEADER_KEY  => 12345} }
    ...
    it "appends the trace information to the outgoing task" do
      ...
      tracer.apply("span1") do |trace|
        ...
        expect(trace_headers).to include({Telemetry::Tracer::TRACE_HEADER_KEY => tracer.id})
        expect(trace_headers).to include({Telemetry::Tracer::SPAN_HEADER_KEY => tracer.current_span_id})
      end
    end
